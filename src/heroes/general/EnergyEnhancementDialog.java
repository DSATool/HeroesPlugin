...
public class EnergyEnhancementDialog {
	...
	public EnergyEnhancementDialog(final Window window, Energy energy, JSONObject hero, int initialTarget) {
		...
		okButton.setOnAction(event -> {
			...
			energy.setSes(Math.max(energy.getSes() - Math.min(ses.getValue(), target.getValue() - energy.getMax()), 0));
			stage.close();
		});
	...
	private int getCalculatedCost(Energy energy, JSONObject hero) {
